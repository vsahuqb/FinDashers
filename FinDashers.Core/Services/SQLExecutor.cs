...
using Npgsql; // Changed from Microsoft.Data.Sqlite
...
public class SQLExecutor
{
    ...
    public async Task<QueryResult> ExecuteScriptAsync(List<string> sqlScript, string domain)
    {
        ...
        try
        {
            ...
            var connectionString = "Host=localhost;Port=5433;Database=adyen_db;Username=postgres;Password=admin;Pooling=true";// GetConnectionString(domain, domainContext.DatabasePath);
            
            _logger.LogDebug("Executing SQL script with {StatementCount} statements for domain '{Domain}'", 
                sqlScript.Count, domain);

            using var connection = new NpgsqlConnection(connectionString); // Changed from SqliteConnection
            ...
            foreach (var statement in sqlScript)
            {
                ...
                using var command = new NpgsqlCommand(statement, connection); // Changed from SqliteCommand
    ...
    {
        // Always use the connection string from DatabaseConfig populated from appsettings
        ...
        {
            throw new InvalidOperationException($"No connection string configured for domain '{domain}' in appsettings.");
        }
    ...
    public async Task<bool> TestConnectionAsync(string domain)
    {
        try
        {
            ...
            using var command = new NpgsqlCommand("SELECT 1", connection); // Changed from SqliteCommand
